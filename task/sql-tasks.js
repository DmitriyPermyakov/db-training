...
async function task_1_20(db) {
  let result = await db.query(`
    WITH
      TotalAmountOfOrder AS
      (SELECT details.OrderID,
              sum(details.UnitPrice * details.Quantity) AS totalSum
      FROM [Order Details] AS details
      GROUP BY details.OrderID),

      RankedAmountOfSales AS
      (SELECT o.EmployeeID,
              (emp.FirstName + ' ' + emp.LastName) AS [Employee Full Name],
              sum(total.totalSum) AS [Amount, $],
              DENSE_RANK() over(
                                ORDER BY sum(total.totalSum) DESC) AS rnk
      FROM Orders AS o
      LEFT JOIN Employees AS emp ON emp.EmployeeID = o.EmployeeID
      LEFT JOIN TotalAmountOfOrder AS total ON o.OrderID = total.OrderID
      GROUP BY o.EmployeeID, (emp.FirstName + ' ' + emp.LastName))

    SELECT rankedSales.EmployeeID,
        rankedSales.[Employee Full Name],
        rankedSales.[Amount, $]
    FROM RankedAmountOfSales AS rankedSales
    WHERE rankedSales.rnk < 2
`);
  return result[0];
}
