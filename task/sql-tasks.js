...
async function task_1_15(db) {
  let result = await db.query(`
  SELECT
	  COUNT(CASE WHEN MONTH(subQuery.OrderDate) = 1 THEN OrderID END) AS January,
	  COUNT(CASE WHEN MONTH(subQuery.OrderDate) = 2 THEN OrderID END) AS February,
	  COUNT(CASE WHEN MONTH(subQuery.OrderDate) = 3 THEN OrderID END) AS March,
	  COUNT(CASE WHEN MONTH(subQuery.OrderDate) = 4 THEN OrderID END) AS April,
	  COUNT(CASE WHEN MONTH(subQuery.OrderDate) = 5 THEN OrderID END) AS May,
	  COUNT(CASE WHEN MONTH(subQuery.OrderDate) = 6 THEN OrderID END) AS June,
	  COUNT(CASE WHEN MONTH(subQuery.OrderDate) = 7 THEN OrderID END) AS July,
	  COUNT(CASE WHEN MONTH(subQuery.OrderDate) = 8 THEN OrderID END) AS August,
	  COUNT(CASE WHEN MONTH(subQuery.OrderDate) = 9 THEN OrderID END) AS September,
	  COUNT(CASE WHEN MONTH(subQuery.OrderDate) = 10 THEN OrderID END) AS October,
	  COUNT(CASE WHEN MONTH(subQuery.OrderDate) = 11 THEN OrderID END) AS November,
	  COUNT(CASE WHEN MONTH(subQuery.OrderDate) = 12 THEN OrderID END) AS December
FROM
  ( SELECT OrderID, OrderDate
    FROM Orders
    WHERE YEAR(OrderDate)= 1997) AS subQuery
`);
}
