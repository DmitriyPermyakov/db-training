...
async function task_1_19(db) {
  ...
  SELECT o.CustomerID,
       cust.CompanyName,
       sum(tot.total) AS [TotalOrdersAmount, $]
  FROM Orders AS o
  LEFT JOIN Customers AS cust ON o.CustomerID = cust.CustomerID
  LEFT JOIN TotalOrderSum AS tot ON o.OrderID = tot.OrderID
  GROUP BY o.CustomerID,
         cust.CompanyName
  HAVING sum(tot.total) > 1000
  ORDER BY [TotalOrdersAmount, $] DESC, o.CustomerID
`);
  return result[0];
}
